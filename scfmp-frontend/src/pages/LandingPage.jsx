...
const BrandMark = ({ compact = false }) => (
  <span className="inline-flex items-center gap-2.5">
    <span className={`grid place-items-center rounded-xl bg-gradient-to-br from-sky-400 to-blue-600 font-bold text-white shadow-lg shadow-blue-950/30 ${compact ? 'h-9 w-9 text-base' : 'h-11 w-11 text-lg'}`} aria-hidden="true">S</span>
...
const Cta = ({ to, children, light = false, className = '' }) => (
  <Link
    to={to}
    className={`focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition duration-200 hover:-translate-y-0.5 ${light ? 'bg-white text-blue-950 shadow-lg shadow-blue-950/20 hover:bg-sky-50' : 'bg-blue-600 text-white shadow-lg shadow-blue-950/15 hover:bg-blue-500'} ${className}`}
  >
...
const PublicProfileImage = ({ member, priority = false }) => {
  ...
  if (!source || failed) {
    return (
      <div className="grid aspect-[4/3] place-items-center bg-gradient-to-br from-blue-100 to-sky-50 text-blue-700">
...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen overflow-x-clip bg-white text-slate-900">
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#071a32]/95 text-white backdrop-blur-lg">
        ...
        {menuOpen && (
          <div className="border-t border-white/10 bg-[#071a32] px-5 py-5 lg:hidden">
      ...
      <main id="main-content">
        <section id="home" className="landing-grid relative isolate overflow-hidden bg-[#071a32] text-white">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_24%,rgba(14,165,233,.27),transparent_24rem),radial-gradient(circle_at_13%_85%,rgba(37,99,235,.24),transparent_27rem)]" aria-hidden="true" />
          ...
          <div className="relative mx-auto grid min-h-[660px] max-w-7xl items-center gap-14 px-5 py-20 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:py-24">
            <div className="landing-reveal max-w-2xl">
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Cta to={platformDestination} light>{isLoading ? t('landing.accessPlatform') : platformLabel}</Cta>
                <a href="#agribridge" className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/25 px-5 py-3 text-sm font-semibold text-white transition hover:border-white/60 hover:bg-white/10">Explore AgriBridge <ArrowDownRight className="h-4 w-4" /></a>
              </div>
        ...
        <section id="team" className="bg-slate-50 py-20 sm:py-28"><div className="mx-auto max-w-7xl px-5 sm:px-8"><SectionIntro eyebrow="Meet our team" title="The people behind SmartBridge and AgriBridge."><p>Only approved public profiles are shown here. Platform permissions, account details, and internal access information remain private.</p></SectionIntro>{teamState === 'loading' ? <div className="mt-10 grid gap-5 md:grid-cols-3">{[1, 2, 3].map((item) => <div key={item} className="animate-pulse overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="aspect-[4/3] bg-slate-200" /><div className="space-y-3 p-6"><div className="h-4 w-2/3 rounded bg-slate-200" /><div className="h-3 w-1/2 rounded bg-slate-100" /></div></div>)}</div> : team.length > 0 ? <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{team.map((member, index) => <article key={`${member.full_name}-${member.position}`} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl"><PublicProfileImage member={member} priority={index < 2} /><div className="p-6"><p className="text-xs font-bold uppercase tracking-[.15em] text-blue-600">{member.position}</p><h3 className="mt-2 text-xl font-semibold text-slate-950">{member.full_name}</h3>{member.biography && <p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-600">{member.biography}</p>}<button onClick={() => setSelectedMember(member)} className="focus-ring mt-5 inline-flex items-center gap-2 text-sm font-semibold text-blue-700 hover:text-blue-500">View profile <ArrowRight className="h-4 w-4" /></button></div></article>)}</div> : <div className="mt-10 rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center"><UsersRound className="mx-auto h-9 w-9 text-blue-600" /><h3 className="mt-4 text-lg font-semibold text-slate-950">Approved team profiles will appear here.</h3><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-600">SmartBridge can publish profiles from the existing Team module when members approve their public visibility.</p></div>}{teamState === 'unavailable' && <p className="mt-5 text-sm text-slate-500">Team profiles are temporarily unavailable. Please check back soon.</p>}</div></section>
