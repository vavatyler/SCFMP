...
const emptyForm = {
  ...
  unit_price: '', harvest_date: '', season: '', production_location: '', notes: '', status: 'recorded',
};
...
const ProductionPage = () => {
  ...
  const liveTotal = form.actual_harvest && form.unit_price
    ? Number(form.actual_harvest) * Number(form.unit_price)
    : 0;
  ...
  const handleSubmit = async (event) => {
    ...
    if (form.production_mode === 'group' && (!form.farmer_group_id || form.farmer_id)) {
      setFormError(t('production.selectProductionGroup'));
      return;
    }
        ...
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          {canManage && (
            <button
              ...
              disabled={!activeCooperativeId || (!farmers.length && !farmerGroups.length)}
              ...
              <thead className="border-b border-sand bg-sand/30 text-xs uppercase tracking-wide text-ink-soft">
                <tr>
                  {[
                    ...
                    t('production.totalValue'), t('production.season'), t('production.productionLocation'),
      ...
      <Modal title={editingRecord ? t('production.editHarvest') : t('production.recordHarvest')} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {form.production_mode === 'group' ? (
            <label className="block text-xs font-medium uppercase tracking-wide text-ink-soft">{t('production.productionGroup')}
              <select
                ...
                className="focus-ring mt-1.5 min-h-11 w-full rounded-lg border border-sand px-3 text-sm disabled:bg-sand/30"
              >
                <option value="">{t('production.selectProductionGroup')}</option>
                {farmerGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}
              </select>
            </label>
          ) : (
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <label className="text-xs font-medium uppercase tracking-wide text-ink-soft">{t('production.expectedProduction')}<input required type="number" min="0.01" step="0.01" value={form.expected_production} onChange={(event) => setForm({ ...form, expected_production: event.target.value })} className="focus-ring mt-1.5 min-h-11 w-full rounded-lg border border-sand px-3 text-sm" /></label>
            <label className="text-xs font-medium uppercase tracking-wide text-ink-soft">{t('production.actualHarvest')}<input required type="number" min="0.01" step="0.01" value={form.actual_harvest} onChange={(event) => setForm({ ...form, actual_harvest: event.target.value })} className="focus-ring mt-1.5 min-h-11 w-full rounded-lg border border-sand px-3 text-sm" /></label>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-xs font-medium uppercase tracking-wide text-ink-soft">{t('production.season')}<input required value={form.season} maxLength={20} onChange={(event) => setForm({ ...form, season: event.target.value })} placeholder="2026A" className="focus-ring mt-1.5 min-h-11 w-full rounded-lg border border-sand px-3 text-sm" /></label>
          ...
          <label className="block text-xs font-medium uppercase tracking-wide text-ink-soft">{t('production.productionLocation')}<input required value={form.production_location} maxLength={255} onChange={(event) => setForm({ ...form, production_location: event.target.value })} className="focus-ring mt-1.5 min-h-11 w-full rounded-lg border border-sand px-3 text-sm" /></label>
